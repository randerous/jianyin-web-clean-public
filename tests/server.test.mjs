...
test("update endpoint exposes only the fixed GitHub release metadata", async () => {
  ...
  const fetchImpl = async (url) => {
    ...
    if (url === "https://api.github.com/repos/randerous/jianyin-web-clean-public/releases?per_page=100") {
      return new Response(JSON.stringify([
        { tag_name: "v1.0.45", published_at: "2026-07-21T00:00:00Z", body: "Latest release" },
    ...
    return new Response(JSON.stringify({
      tag_name: "v1.0.45",
      html_url: "https://github.com/randerous/jianyin-web-clean-public/releases/tag/v1.0.45",
      published_at: "2026-07-21T00:00:00Z",
      body: "Latest release",
      assets: [
        {
          name: "app-release.apk",
          browser_download_url: "https://github.com/randerous/jianyin-web-clean-public/releases/download/v1.0.45/app-release.apk",
          digest: "sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
          size: 123
        },
        {
          name: "jianyin-windows-launcher.exe",
          browser_download_url: "https://github.com/randerous/jianyin-web-clean-public/releases/download/v1.0.45/jianyin-windows-launcher.exe",
  ...
  assert.equal(first.body.currentVersion, "1.0.44");
  assert.equal(first.body.latestVersion, "1.0.45");
  assert.equal(first.body.available, true);
  assert.deepEqual(first.body.releaseNotes.map((note) => note.tag), ["v1.0.45"]);
  ...
  assert.equal(second.body.tag, "v1.0.45");
