...
test("v1.0.36 release metadata is synchronized across web and Android", async () => {
  const [packageJson, gradle] = await Promise.all([
    readFile(resolve(root, "package.json"), "utf8"),
    readFile(resolve(root, "android/app/build.gradle"), "utf8")
  ]);

  assert.equal(JSON.parse(packageJson).version, "1.0.36");
  assert.match(gradle, /versionCode 37/);
  assert.match(gradle, /versionName "1\.0.36"/);
});
