...
test("WebAudio EQ graph applies to the playing element; bypass disables processing", async ({ page }) => {
  type EqDebug = {
  ...
  // headless 下 MediaElementSource 会冻结元素时钟，图状态断言不依赖 currentTime
  const expectElementPlaying = () => expect.poll(() => page.locator("audio").evaluate((audio: HTMLAudioElement) => audio.paused)).toBe(false);

  // 先启用 hiFi，再播放（点击手势内接线）
  const dialog = await openSettings(page);
  await dialog.getByLabel("均衡器预设").selectOption("hiFi");
  await dialog.getByRole("button", { name: "关闭" }).click();
  await expect(page.getByRole("dialog", { name: "设置" })).toHaveCount(0);
  await playFirstHomeSong(page);
  await expectElementPlaying();

  // 默认 hiFi 微笑曲线：图已激活、非旁路、各段增益与预设表一致
  await expect.poll(eqInfo).toMatchObject({ contextState: "running", preset: "hiFi", intensity: 100, bypass: false });
  await expect.poll(async () => (await eqInfo())?.bandGains).toEqual([3, 3, 2, 1, 0, -1.5, 0, 1.5, 3, 4]);

  // 播放中切换预设：增益即时重应用（vocal 的 1k/2k 段 +3）
  const dialog2 = await openSettings(page);
  await dialog2.getByLabel("均衡器预设").selectOption("vocal");
  await expect.poll(eqInfo).toMatchObject({ preset: "vocal", bypass: false });
  await expect.poll(async () => (await eqInfo())?.bandGains).toEqual([-2, -1, 0, 1, 2, 3, 3, 2, 1, 0]);

  // 强度缩放：vocal 50% → 各段减半
  await dialog2.getByLabel("均衡器强度").fill("50");
  await expect.poll(async () => (await eqInfo())?.bandGains).toEqual([-1, -0.5, 0, 0.5, 1, 1.5, 1.5, 1, 0.5, 0]);

  // 原声（关闭）：拓扑旁路（链断开、零处理；增益保留旧值但不在信号路径上）
  await dialog2.getByLabel("均衡器预设").selectOption("none");
  await expect.poll(eqInfo).toMatchObject({ preset: "none", bypass: true });

  // 从旁路恢复：链重新接入（回归：source 直连必须只断开到 destination 而不是全断）
  await dialog2.getByLabel("均衡器预设").selectOption("hiFi");
  await expect.poll(eqInfo).toMatchObject({ preset: "hiFi", bypass: false, intensity: 50 });
  await expect.poll(async () => (await eqInfo())?.bandGains).toEqual([1.5, 1.5, 1, 0.5, 0, -0.75, 0, 0.75, 1.5, 2]);
  await expectElementPlaying();
});
