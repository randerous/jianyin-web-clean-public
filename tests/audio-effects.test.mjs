...
test("normalizeEqPreset: known ids pass through, unknown/missing default to hiFi", () => {
...
test("node environment (no window): all graph functions are safe no-ops", () => {
  assert.equal(typeof window, "undefined");
  assert.doesNotThrow(() => setAudioEffects("rock", 60));
  assert.doesNotThrow(() => ensureAudioEffects(null));
  assert.doesNotThrow(() => applyAudioEffects());
